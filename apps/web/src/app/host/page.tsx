...
// Photo Finish data shape
interface PhotoFinishData {
  round: number;
  phase: 'answer' | 'reveal';
  finishLine: number;
  question: string;
...
// Blitz Quiz data shape
interface BlitzQuizData {
  round: number;
  totalRounds: number;
...
// Synonym Sprint data shape
interface SynonymSprintData {
  round: number;
  totalRounds: number;
  phase: 'pick' | 'reveal';
  word: string;
...
interface CapitalConfusionData {
  round: number;
  totalRounds: number;
  phase: 'pick' | 'reveal';
  city: string;
...
// Beat Drop data shape
interface BeatDropData {
  round: number;
  totalRounds: number;
...
// Bid or Bust data shape
interface BidOrBustData {
  round: number;
  totalRounds: number;
  phase: 'bid' | 'reveal';
  prompt: string;
...
// Combo Chain data shape
interface ComboChainData {
  round: number;
  totalRounds: number;
  phase: 'pick' | 'reveal';
  ruleName: string;
...
// Broken Telephone data shape
interface BrokenTelephoneData {
  round: number;
  totalRounds: number;
...
const GAME_LABELS: Record<string, string> = {
  ...
  brokentelephone: '📞 Broken Telephone',
  beatdrop: '🎵 Beat Drop',
  bidorbust: '💸 Bid or Bust',
  mapattack: '🌍 Map Attack',
  ...
  acronymdecoder: '🔠 Acronym Decoder',
  biggerorsmaller: '📏 Bigger or Smaller',
  famousquotes: '💬 Famous Quotes',
  animalornot: '🦎 Animal or Not',
  wordassociation: '🔗 Word Association',
  decadedetector: '📅 Decade Detector',
  emojitranslate: '🎬 Emoji Translate',
  speedmath: '🧮 Speed Math',
  flagfrenzy: '🏳️ Flag Frenzy',
  oddwordout: '🔍 Odd Word Out',
  tonguetwister: '👅 Tongue Twister',
  colorspy: '🎨 Color Spy',
  reversespell: '🔄 Reverse Spell',
  whosaidit: '💬 Who Said It?',
  sounditout: '🔊 Sound It Out',
  firstandlast: '🔤 First & Last',
  mythorfact: '🤔 Myth or Fact',
  lettercount: '🔢 Letter Count',
  emojichain: '🔗 Emoji Chain',
  capitalconfusion: '🏛️ Capital Confusion',
  syllablecount: '🗣️ Syllable Count',
  oddemojiout: '🎭 Odd Emoji Out',
};
...
function GameView({
  ...
  if (gameId === 'combochain' && gameState) {
    return <ComboChainHostView state={gameState} players={players} />;
  }

  if (gameId === 'brokentelephone' && gameState) {
    return <BrokenTelephoneHostView state={gameState} players={players} />;
  }

  if (gameId === 'beatdrop' && gameState) {
    return <BeatDropHostView state={gameState} players={players} />;
  }

  if (gameId === 'bidorbust' && gameState) {
    return <BidOrBustHostView state={gameState} players={players} />;
  }

  if (gameId === 'mapattack' && gameState) {
    return <MapAttackHostView state={gameState} players={players} />;
  }

  if (gameId === 'oddoneout' && gameState) {
    return <OddOneOutHostView state={gameState} players={players} />;
  }

  if (gameId === 'emojistory' && gameState) {
    return <EmojiStoryHostView state={gameState} players={players} />;
  }
  ...
  if (gameId === 'countdown' && gameState) {
    return <CountdownHostView state={gameState} players={players} />;
  }

  if (gameId === 'vibecheck' && gameState) {
    return <VibeCheckHostView state={gameState} players={players} />;
  }

  if (gameId === 'closestguess' && gameState) {
    return <ClosestGuessHostView state={gameState} players={players} />;
  }

  if (gameId === 'blitzquiz' && gameState) {
    return <BlitzQuizHostView state={gameState} players={players} />;
  }

  if (gameId === 'synonymsprint' && gameState) {
    return <SynonymSprintHostView state={gameState} players={players} />;
  }
  ...
  if (gameId === 'spellbee' && gameState) {
    return <SpellBeeHostView state={gameState} players={players} />;
  }

  if (gameId === 'snapjudge' && gameState) {
    return <SnapJudgeHostView state={gameState} players={players} />;
  }

  if (gameId === 'guesstheyear' && gameState) {
    return <GuessTheYearHostView state={gameState} players={players} />;
  }

  if (gameId === 'alphabetrace' && gameState) {
    return <AlphabetRaceHostView state={gameState} players={players} />;
  }

  if (gameId === 'top5' && gameState) {
    return <Top5HostView state={gameState} players={players} />;
  }

  if (gameId === 'spotthefake' && gameState) {
    return <SpotTheFakeHostView state={gameState} players={players} />;
  }

  if (gameId === 'thisorthat' && gameState) {
    return <ThisOrThatHostView state={gameState} players={players} />;
  }

  if (gameId === 'numbercrunch' && gameState) {
    return <NumberCrunchHostView state={gameState} players={players} />;
  }

  if (gameId === 'finishthelyric' && gameState) {
    return <FinishTheLyricHostView state={gameState} players={players} />;
  }

  if (gameId === 'capitalquiz' && gameState) {
    return <CapitalQuizHostView state={gameState} players={players} />;
  }

  if (gameId === 'emojimath' && gameState) {
    return <EmojiMathHostView state={gameState} players={players} />;
  }

  if (gameId === 'beforeorafter' && gameState) {
    return <BeforeOrAfterHostView state={gameState} players={players} />;
  }
  if (gameId === 'rhymetime' && gameState) {
    return <RhymeTimeHostView state={gameState} players={players} />;
  }
  if (gameId === 'missingletter' && gameState) {
    return <MissingLetterHostView state={gameState} players={players} />;
  }
  if (gameId === 'soundalike' && gameState) {
    return <SoundAlikeHostView state={gameState} players={players} />;
  }
  if (gameId === 'categoryblitz' && gameState) {
    return <CategoryBlitzHostView state={gameState} players={players} />;
  }
  if (gameId === 'acronymdecoder' && gameState) {
    return <AcronymDecoderHostView state={gameState} players={players} />;
  }
  if (gameId === 'biggerorsmaller' && gameState) {
    return <BiggerOrSmallerHostView state={gameState} players={players} />;
  }
  if (gameId === 'famousquotes' && gameState) {
    return <FamousQuotesHostView state={gameState} players={players} />;
  }
  if (gameId === 'animalornot' && gameState) {
    return <AnimalOrNotHostView state={gameState} players={players} />;
  }
  if (gameId === 'wordassociation' && gameState) {
    return <WordAssociationHostView state={gameState} players={players} />;
  }
  if (gameId === 'decadedetector' && gameState) {
    return <DecadeDetectorHostView state={gameState} players={players} />;
  }
  if (gameId === 'emojitranslate' && gameState) {
    return <EmojiTranslateHostView state={gameState} players={players} />;
  }
  if (gameId === 'speedmath' && gameState) {
    return <SpeedMathHostView state={gameState} players={players} />;
  }
  if (gameId === 'flagfrenzy' && gameState) {
    return <FlagFrenzyHostView state={gameState} players={players} />;
  }
  if (gameId === 'oddwordout' && gameState) {
    return <OddWordOutHostView state={gameState} players={players} />;
  }
  if (gameId === 'tonguetwister' && gameState) {
    return <TongueTwisterHostView state={gameState} players={players} />;
  }
  if (gameId === 'colorspy' && gameState) {
    return <ColorSpyHostView state={gameState} players={players} />;
  }
  if (gameId === 'reversespell' && gameState) {
    return <ReverseSpellHostView state={gameState} players={players} />;
  }
  if (gameId === 'whosaidit' && gameState) {
    return <WhoSaidItHostView state={gameState} players={players} />;
  }
  if (gameId === 'sounditout' && gameState) {
    return <SoundItOutHostView state={gameState} players={players} />;
  }
  if (gameId === 'firstandlast' && gameState) {
    return <FirstAndLastHostView state={gameState} players={players} />;
  }
  if (gameId === 'mythorfact' && gameState) {
    return <MythOrFactHostView state={gameState} players={players} />;
  }
  if (gameId === 'lettercount' && gameState) {
    return <LetterCountHostView state={gameState} players={players} />;
  }
  if (gameId === 'emojichain' && gameState) {
    return <EmojiChainHostView state={gameState} players={players} />;
  }
  if (gameId === 'capitalconfusion' && gameState) {
    return <CapitalConfusionHostView state={gameState} players={players} />;
  }
  if (gameId === 'syllablecount' && gameState) {
    return <SyllableCountHostView state={gameState} players={players} />;
  }
  if (gameId === 'oddemojiout' && gameState) {
    return <OddEmojiOutHostView state={gameState} players={players} />;
  }
...
const HS_COLORS = ['#ef4444', '#3b82f6', '#22c55e', '#f59e0b'];
const HS_LABELS = ['A', 'B', 'C', 'D'];

function HotSeatHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as HotSeatData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const hotPlayer = nonHostPlayers.find((p) => p.id === data.hotSeatPlayerId);
  const timerFraction = data.answerMs / 8_000;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }} />
        {data.phase === 'active' && (
          <span style={{ fontSize: '1.25rem', fontWeight: 800, color: timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444' }}>
            {Math.ceil(data.answerMs / 1000)}s
          </span>
        )}
        <span style={{ fontSize: '1.5rem' }}>🔥</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        {hotPlayer && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: AVATAR_COLOR_HEX[hotPlayer.avatarColor], border: '2px solid #f59e0b' }} />
            <span style={{ fontSize: '1.5rem', fontWeight: 800, color: '#f59e0b' }}>{hotPlayer.name}</span>
            <span style={{ color: '#6b7280', fontSize: '0.9rem' }}>is in the hot seat!</span>
          </div>
        )}

        {/* Prompt */}
        <h2 style={{ fontSize: '1.8rem', fontWeight: 900, color: '#a78bfa', textAlign: 'center', margin: 0 }}>
          {data.prompt}
        </h2>
        <p style={{ color: '#6b7280', fontSize: '0.85rem', margin: 0 }}>
          Tap all correct answers — avoid the wrong one!
        </p>

        {/* Options grid */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 500 }}>
          {data.options.map((opt, i) => {
            const isSelected = data.selectedIndices.includes(i);
            const isCorrect = data.correctIndices?.includes(i);
            const isReveal = data.phase === 'reveal';
            const bg = isReveal
              ? isCorrect ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.1)'
              : isSelected ? 'rgba(167,139,250,0.2)' : `${HS_COLORS[i]}10`;
            const border = isReveal
              ? isCorrect ? '#22c55e' : '#ef444466'
              : isSelected ? '#a78bfa' : `${HS_COLORS[i]}44`;
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '0.65rem', color: '#6b7280', fontWeight: 600 }}>{HS_LABELS[i]}</span>
                <p style={{ fontSize: '1.1rem', fontWeight: 700, color: '#f0f0ff', margin: '0.25rem 0 0' }}>{opt}</p>
                {isReveal && isCorrect && <span style={{ fontSize: '0.8rem' }}>✅</span>}
                {isReveal && !isCorrect && isSelected && <span style={{ fontSize: '0.8rem' }}>❌</span>}
                {isSelected && !isReveal && <span style={{ fontSize: '0.8rem' }}>✓</span>}
              </div>
            );
          })}
        </div>
        ...
        {data.phase === 'reveal' && (
          <div style={{ textAlign: 'center' }}>
            {data.isPerfect ? (
              <p style={{ fontSize: '1.3rem', fontWeight: 800, color: '#22c55e' }}>🎯 PERFECT!</p>
            ) : (
              <p style={{ fontSize: '1rem', color: '#9ca3af' }}>
                {data.correctCount} correct, {data.wrongCount} wrong
              </p>
            )}
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: '0.5rem', padding: '1rem 2rem', justifyContent: 'center', flexWrap: 'wrap', flexShrink: 0 }}>
        {nonHostPlayers.map((p) => {
          const isHot = p.id === data.hotSeatPlayerId;
          return (
            <div key={p.id} style={{
              display: 'flex', alignItems: 'center', gap: '0.3rem',
              padding: '0.3rem 0.6rem', borderRadius: 10,
              background: isHot ? 'rgba(245,158,11,0.15)' : 'rgba(255,255,255,0.03)',
              border: `1px solid ${isHot ? '#f59e0b44' : 'rgba(255,255,255,0.06)'}`,
            }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor] }} />
              <span style={{ fontSize: '0.7rem', fontWeight: 600, color: '#f0f0ff' }}>{p.name}</span>
              {isHot && <span style={{ fontSize: '0.7rem' }}>🔥</span>}
            </div>
          );
        })}
      </div>
...
// ── Combo Chain Host View ──────────────────────────────────────────────────────

function ComboChainHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as ComboChainData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '2rem', gap: '1.5rem', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#312e81', color: '#818cf8' }}>
            {data.ruleName}
          </span>
          <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(data.round / data.totalRounds) * 100}%`, background: '#6366f1', borderRadius: 4, transition: 'width 0.3s' }} />
          </div>
          {!isReveal && (
            <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor, minWidth: 48, textAlign: 'right' }}>
              {Math.ceil(data.pickMs / 1000)}s
            </span>
          )}
        </div>
        ...
        {/* Chain visualization */}
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', padding: '0.5rem' }}>
          {data.chain.map((word, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              {i > 0 && <span style={{ color: '#4b5563', fontSize: '1.2rem' }}>→</span>}
              <span style={{ padding: '0.4rem 0.8rem', borderRadius: 8, background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.3)', color: '#a5b4fc', fontWeight: 700, fontSize: '1rem' }}>
                {word}
              </span>
            </div>
          ))}
          {!isReveal && <span style={{ color: '#4b5563', fontSize: '1.2rem' }}>→ ?</span>}
        </div>

        {/* Rule description */}
        <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: '0.9rem' }}>{data.ruleDescription}</p>

        {/* Current word + options */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem' }}>
          <h1 style={{ fontSize: '2.5rem', fontWeight: 900, color: '#f0f0ff' }}>{data.currentWord}</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 700 }}>
            {data.options.map((option, i) => {
              ...
              let opacity = 1;
              if (isReveal) {
                if (isCorrect) { bg = `${color}33`; border = `2px solid ${color}`; }
                else { opacity = 0.4; }
              }
              return (
                <div key={i} style={{ padding: '1rem', borderRadius: 12, background: bg, border, opacity, textAlign: 'center', transition: 'all 0.3s' }}>
                  <span style={{ fontWeight: 800, color, fontSize: '0.85rem' }}>{ANSWER_LABELS[i]}</span>
                  <p style={{ fontWeight: 700, fontSize: '1.2rem', color: '#f0f0ff', marginTop: '0.3rem' }}>{option}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* Sidebar scoreboard */}
      <div style={{ background: '#111127', borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '1.5rem 1rem', overflowY: 'auto' }}>
        <h3 style={{ fontSize: '0.85rem', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', marginBottom: '1rem' }}>Scoreboard</h3>
        {sorted.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#f59e0b' : '#6b7280', width: 20 }}>#{i + 1}</span>
            <div style={{ width: 24, height: 24, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor], flexShrink: 0 }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff', flex: 1 }}>{p.name}</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#a78bfa' }}>{state.scores[p.id] ?? 0}</span>
          </div>
        ))}
      </div>
    </div>
...
// ── Broken Telephone Host View ────────────────────────────────────────────────

function BrokenTelephoneHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as BrokenTelephoneData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#7c2d12', color: '#fb923c' }}>
          {data.category}
        </span>
        <div style={{ flex: 1, height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(data.round / data.totalRounds) * 100}%`, background: '#f97316', borderRadius: 4, transition: 'width 0.3s' }} />
        </div>
        {!isReveal && (
          <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor, minWidth: 48, textAlign: 'right' }}>
            {Math.ceil(data.pickMs / 1000)}s
      ...
      {/* Timer bar */}
      {!isReveal && (
        <div style={{ height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden', margin: '0 2rem', flexShrink: 0 }}>
          <div style={{ height: '100%', width: `${timerFraction * 100}%`, background: timerColor, borderRadius: 4, transition: 'width 0.1s linear' }} />
        </div>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem', padding: '0 2rem' }}>
        ...
        {data.phase === 'pick_drawing' && data.promptPhrase && (
          <>
            <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>Which drawing represents this phrase?</p>
            <h1 style={{ fontSize: '2rem', fontWeight: 900, color: '#f0f0ff', textAlign: 'center' }}>&quot;{data.promptPhrase}&quot;</h1>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', maxWidth: 600 }}>
              {data.drawingOptions.map((emoji, i) => (
                <div key={i} style={{ padding: '1.5rem', borderRadius: 16, background: '#1a1a2e', border: `2px solid ${ANSWER_COLORS[i]}44`, textAlign: 'center' }}>
                  <span style={{ fontWeight: 800, color: ANSWER_COLORS[i], fontSize: '0.85rem' }}>{ANSWER_LABELS[i]}</span>
                  <p style={{ fontSize: '3rem', marginTop: '0.5rem' }}>{emoji}</p>
                </div>
              ))}
            </div>
          </>
        ...
        {data.phase === 'pick_phrase' && data.promptDrawing && (
          <>
            <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>What does this drawing mean?</p>
            <div style={{ fontSize: '5rem', padding: '1rem 2rem', background: '#1a1a2e', borderRadius: 16, border: '2px solid rgba(249,115,22,0.3)' }}>
              {data.promptDrawing}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', maxWidth: 700, width: '100%' }}>
              {data.phraseOptions.map((phrase, i) => (
                <div key={i} style={{ padding: '1rem', borderRadius: 12, background: '#1a1a2e', border: `2px solid ${ANSWER_COLORS[i]}44`, textAlign: 'center' }}>
                  <span style={{ fontWeight: 800, color: ANSWER_COLORS[i], fontSize: '0.85rem' }}>{ANSWER_LABELS[i]}</span>
                  <p style={{ fontWeight: 700, fontSize: '1rem', color: '#f0f0ff', marginTop: '0.3rem' }}>{phrase}</p>
                </div>
              ))}
            </div>
          </>
        ...
        {isReveal && (
          <>
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#f97316' }}>The Telephone Chain</h2>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ color: '#6b7280', fontSize: '0.75rem' }}>Original</p>
                <p style={{ fontSize: '1.2rem', fontWeight: 700, color: '#f0f0ff' }}>&quot;{data.originalPhrase}&quot;</p>
              </div>
              <span style={{ fontSize: '2rem', color: '#4b5563' }}>→</span>
              {data.chosenDrawing && (
                <div style={{ textAlign: 'center' }}>
                  <p style={{ color: '#6b7280', fontSize: '0.75rem' }}>Drawing</p>
                  <p style={{ fontSize: '3rem' }}>{data.chosenDrawing}</p>
                </div>
              )}
              <span style={{ fontSize: '2rem', color: '#4b5563' }}>→</span>
              {data.chosenPhrase && (
                <div style={{ textAlign: 'center' }}>
                  <p style={{ color: '#6b7280', fontSize: '0.75rem' }}>Final</p>
                  <p style={{ fontSize: '1.2rem', fontWeight: 700, color: data.chosenPhrase === data.originalPhrase ? '#22c55e' : '#ef4444' }}>
                    &quot;{data.chosenPhrase}&quot;
                  </p>
                </div>
              )}
        ...
        {!isReveal && (
          <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>
            {data.pickedPlayerIds.length} / {nonHostPlayers.length} answered
          </p>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', padding: '1rem 2rem', justifyContent: 'center', background: 'rgba(15,15,26,0.8)' }}>
        {sorted.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#f97316' : '#6b7280' }}>#{i + 1}</span>
            <div style={{ width: 16, height: 16, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor] }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff' }}>{p.name}</span>
            <span style={{ fontSize: '0.8rem', color: '#6b7280' }}>{state.scores[p.id] ?? 0}</span>
          </div>
...
function BeatDropHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as BeatDropData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const activePlayer = nonHostPlayers.find((p) => p.id === data.activePlayerId);

  const phaseLabel = data.phase === 'place' ? 'Add Your Sounds!' : data.phase === 'playback' ? 'Listen to the Beat!' : data.phase === 'vote' ? 'Vote for MVP!' : 'Results';
  const phaseColor = data.phase === 'place' ? '#f59e0b' : data.phase === 'playback' ? '#22c55e' : data.phase === 'vote' ? '#8b5cf6' : '#ec4899';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(data.round / data.totalRounds) * 100}%`, background: '#8b5cf6', borderRadius: 4, transition: 'width 0.3s' }} />
        </div>
        <span style={{ fontSize: '1.5rem' }}>🎵</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        <h2 style={{ fontSize: '2rem', fontWeight: 800, color: phaseColor }}>{phaseLabel}</h2>

        {data.phase === 'place' && activePlayer && (
          <p style={{ fontSize: '1.2rem', color: '#a78bfa' }}>
            <span style={{ color: AVATAR_COLOR_HEX[activePlayer.avatarColor], fontWeight: 700 }}>{activePlayer.name}</span>&apos;s turn
          </p>
        )}

        {/* Beat grid visualization */}
        <div style={{ display: 'flex', gap: '4px', padding: '1rem', background: '#111127', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)' }}>
          {data.grid.map((slot, beatIdx) => (
            <div key={beatIdx} style={{
              width: 48, minHeight: 80, borderRadius: 6,
              background: data.phase === 'playback' && data.playbackBeatIndex === beatIdx ? 'rgba(139,92,246,0.3)' : 'rgba(255,255,255,0.03)',
              border: data.phase === 'playback' && data.playbackBeatIndex === beatIdx ? '2px solid #8b5cf6' : '1px solid rgba(255,255,255,0.06)',
              display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: '2px', padding: '4px',
              transition: 'all 0.15s',
            }}>
              {slot.map((s, i) => (
                <div key={i} style={{
                  width: 36, height: 16, borderRadius: 4,
                  background: data.soundColors[s.sound] ?? '#666',
                  fontSize: '0.55rem', fontWeight: 700, color: '#fff',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  {(data.soundLabels[s.sound] ?? s.sound).split(' ')[0]}
                </div>
              ))}
            </div>
          ))}
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          {['A', 'B', 'C', 'D'].map((s) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
              <div style={{ width: 12, height: 12, borderRadius: 3, background: data.soundColors[s] ?? '#666' }} />
              <span style={{ fontSize: '0.75rem', color: '#9ca3af' }}>{data.soundLabels[s]}</span>
            </div>
          ))}
        </div>

        {/* Vote phase: show player options */}
        {data.phase === 'vote' && (
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
            {data.turnOrder.map((pid, i) => {
              const p = nonHostPlayers.find((pl) => pl.id === pid);
              if (!p) return null;
              const isMVP = data.roundMVP === pid;
              const voteCount = data.voteResults[pid] ?? 0;
              return (
                <div key={pid} style={{
                  padding: '1rem', borderRadius: 12, textAlign: 'center',
                  background: isMVP ? 'rgba(139,92,246,0.2)' : 'rgba(255,255,255,0.05)',
                  border: `2px solid ${ANSWER_COLORS[i] ?? '#666'}44`,
                  minWidth: 80,
                }}>
                  <span style={{ fontWeight: 800, color: ANSWER_COLORS[i], fontSize: '0.85rem' }}>{ANSWER_LABELS[i]}</span>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor], margin: '0.5rem auto' }} />
                  <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff' }}>{p.name}</span>
                  {data.phase === 'vote' && voteCount > 0 && (
                    <p style={{ fontSize: '0.7rem', color: '#8b5cf6' }}>{voteCount} votes</p>
                  )}
                </div>
              );
            })}
        ...
        {/* Reveal: MVP */}
        {data.phase === 'reveal' && data.roundMVP && (
          <div style={{ textAlign: 'center' }}>
            <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>Round MVP</p>
            <p style={{ fontSize: '1.5rem', fontWeight: 800, color: '#8b5cf6' }}>
              {nonHostPlayers.find((p) => p.id === data.roundMVP)?.name ?? 'Unknown'}
            </p>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', padding: '1rem 2rem', justifyContent: 'center', background: 'rgba(15,15,26,0.8)' }}>
        {sorted.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#8b5cf6' : '#6b7280' }}>#{i + 1}</span>
            <div style={{ width: 16, height: 16, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor] }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff' }}>{p.name}</span>
            <span style={{ fontSize: '0.8rem', color: '#6b7280' }}>{state.scores[p.id] ?? 0}</span>
          </div>
...
// ── Bid or Bust Host View ─────────────────────────────────────────────────────

function BidOrBustHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as BidOrBustData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.bidMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '2rem', gap: '1.5rem', overflow: 'hidden' }}>
        ...
        {/* Prompt + options */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem' }}>
          <h1 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.5rem)', fontWeight: 900, textAlign: 'center', color: '#f0f0ff', maxWidth: 800 }}>
            {data.prompt}
          </h1>

          {isReveal && data.actualValue !== null && (
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: '#6b7280', fontSize: '0.85rem' }}>Actual Answer</p>
              <p style={{ fontSize: '3rem', fontWeight: 900, color: '#22c55e' }}>
                {data.actualValue.toLocaleString()} {data.unit}
              </p>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 700 }}>
            {data.ranges.map((range, i) => {
              const color = ANSWER_COLORS[i]!;
              const isClosest = isReveal && data.closestPlayerId && data.playerBids[data.closestPlayerId] === i;
              const isExact = isReveal && data.exactPlayerIds.some((pid) => data.playerBids[pid] === i);
              const isBust = isReveal && data.bustPlayerIds.some((pid) => data.playerBids[pid] === i);
              let bg = '#1a1a2e';
              let border = `2px solid ${color}44`;
              if (isReveal) {
                if (isExact) { bg = 'rgba(34,197,94,0.2)'; border = '2px solid #22c55e'; }
                else if (isClosest) { bg = `${color}22`; border = `2px solid ${color}`; }
                else if (isBust) { bg = 'rgba(239,68,68,0.15)'; border = '2px solid #ef444466'; }
              }
              return (
                <div key={i} style={{ padding: '1.25rem', borderRadius: 12, background: bg, border, textAlign: 'center', transition: 'all 0.3s' }}>
                  <span style={{ fontWeight: 800, color, fontSize: '0.85rem' }}>{ANSWER_LABELS[i]}</span>
                  <p style={{ fontWeight: 700, fontSize: '1.5rem', color: '#f0f0ff', marginTop: '0.3rem' }}>{range}</p>
                  {isReveal && isBust && <p style={{ fontSize: '0.75rem', color: '#ef4444', fontWeight: 700 }}>BUST!</p>}
                  {isReveal && isExact && <p style={{ fontSize: '0.75rem', color: '#22c55e', fontWeight: 700 }}>EXACT!</p>}
                  {isReveal && isClosest && !isExact && <p style={{ fontSize: '0.75rem', color: '#f59e0b', fontWeight: 700 }}>CLOSEST!</p>}
                </div>
              );
            })}
          </div>
          ...
          {!isReveal && (
            <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>
              {data.bidderIds.length} / {nonHostPlayers.length} bid
            </p>
          )}
        </div>
      </div>
      ...
      <div style={{ background: '#111127', borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '1.5rem 1rem', overflowY: 'auto' }}>
        <h3 style={{ fontSize: '0.85rem', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', marginBottom: '1rem' }}>Scoreboard</h3>
        {sorted.map((p, i) => {
          const isBust = data.bustPlayerIds.includes(p.id);
          const isExact = data.exactPlayerIds.includes(p.id);
          return (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#f59e0b' : '#6b7280', width: 20 }}>#{i + 1}</span>
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor], flexShrink: 0 }} />
              <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff', flex: 1 }}>{p.name}</span>
              {isReveal && isExact && <span style={{ fontSize: '0.65rem', color: '#22c55e' }}>EXACT</span>}
              {isReveal && isBust && <span style={{ fontSize: '0.65rem', color: '#ef4444' }}>BUST</span>}
              <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#a78bfa' }}>{state.scores[p.id] ?? 0}</span>
            </div>
          );
        })}
...
// ── Map Attack Host View ──────────────────────────────────────────────────────

function MapAttackHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as MapAttackData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.guessMs / 10_000;
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '2rem', gap: '1.5rem', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
          <span style={{ color: '#6b7280', fontSize: '0.875rem', fontWeight: 600 }}>
            Q{data.round} / {data.totalRounds}
          </span>
          <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#14532d', color: '#22c55e', textTransform: 'capitalize' }}>
            {data.category}
          </span>
          <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(data.round / data.totalRounds) * 100}%`, background: '#22c55e', borderRadius: 4, transition: 'width 0.3s' }} />
          </div>
          {!isReveal && (
            <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor, minWidth: 48, textAlign: 'right' }}>
              {Math.ceil(data.guessMs / 1000)}s
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem' }}>
          <div style={{ fontSize: '5rem' }}>{data.emoji}</div>
          <h1 style={{ fontSize: 'clamp(1.3rem, 3vw, 2.2rem)', fontWeight: 900, textAlign: 'center', color: '#f0f0ff', maxWidth: 800 }}>
            {data.prompt}
          </h1>
          ...
          {!isReveal && (
            <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>
              {data.guessedPlayerIds.length} / {nonHostPlayers.length} answered
            </p>
          )}
        </div>
      </div>
      ...
      <div style={{ background: '#111127', borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '1.5rem 1rem', overflowY: 'auto' }}>
        ...
        {sorted.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#22c55e' : '#6b7280', width: 20 }}>#{i + 1}</span>
            <div style={{ width: 24, height: 24, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor], flexShrink: 0 }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff', flex: 1 }}>{p.name}</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#a78bfa' }}>{state.scores[p.id] ?? 0}</span>
          </div>
...
// ── Odd One Out Host View ─────────────────────────────────────────────────────

function OddOneOutHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as OddOneOutData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '2rem', gap: '1.5rem', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem' }}>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: '0.9rem', color: '#9ca3af', marginBottom: '0.5rem' }}>Category hint: <strong style={{ color: '#f97316' }}>{data.category}</strong></p>
            <h1 style={{ fontSize: '2rem', fontWeight: 900, color: '#f0f0ff' }}>Which one doesn&apos;t belong?</h1>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', width: '100%', maxWidth: 700 }}>
            {data.items.map((item, i) => {
              const color = ANSWER_COLORS[i]!;
              const isOdd = isReveal && data.oddIndex === i;
              let bg = '#1a1a2e';
              let border = `2px solid ${color}44`;
              if (isReveal) {
                if (isOdd) { bg = 'rgba(239,68,68,0.2)'; border = '2px solid #ef4444'; }
                else { bg = 'rgba(34,197,94,0.1)'; border = '2px solid rgba(34,197,94,0.3)'; }
              }
              return (
                <div key={i} style={{ padding: '1.5rem', borderRadius: 16, background: bg, border, textAlign: 'center', transition: 'all 0.3s' }}>
                  <span style={{ fontWeight: 800, color, fontSize: '0.85rem' }}>{ANSWER_LABELS[i]}</span>
                  <p style={{ fontWeight: 800, fontSize: '1.5rem', color: '#f0f0ff', marginTop: '0.5rem' }}>{item}</p>
                  {isReveal && isOdd && <p style={{ fontSize: '0.75rem', color: '#ef4444', fontWeight: 700, marginTop: '0.3rem' }}>ODD ONE OUT!</p>}
                </div>
              );
            })}
          </div>

          {isReveal && data.explanation && (
            <p style={{ color: '#9ca3af', fontSize: '1rem', fontStyle: 'italic' }}>{data.explanation}</p>
          )}
      ...
      <div style={{ background: '#111127', borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '1.5rem 1rem', overflowY: 'auto' }}>
        ...
        {sorted.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#f97316' : '#6b7280', width: 20 }}>#{i + 1}</span>
            <div style={{ width: 24, height: 24, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor], flexShrink: 0 }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff', flex: 1 }}>{p.name}</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#a78bfa' }}>{state.scores[p.id] ?? 0}</span>
          </div>
...
// ── Emoji Story Host View ─────────────────────────────────────────────────────

function EmojiStoryHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as EmojiStoryData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.guessMs / 12_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', flexDirection: 'column', padding: '2rem', gap: '1.5rem', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#312e81', color: '#818cf8' }}>{data.category}</span>
          <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(data.round / data.totalRounds) * 100}%`, background: '#8b5cf6', borderRadius: 4, transition: 'width 0.3s' }} />
          </div>
          {!isReveal && <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor, minWidth: 48, textAlign: 'right' }}>{Math.ceil(data.guessMs / 1000)}s</span>}
        </div>
        {!isReveal && (
          <div style={{ height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden', flexShrink: 0 }}>
            <div style={{ height: '100%', width: `${timerFraction * 100}%`, background: timerColor, borderRadius: 4, transition: 'width 0.1s linear' }} />
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem' }}>
          <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>What do these emojis represent?</p>
          <div style={{ fontSize: '5rem', letterSpacing: '0.3rem', padding: '1rem 2rem', background: '#111127', borderRadius: 16, border: '1px solid rgba(139,92,246,0.3)' }}>
            {data.emojis}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 700 }}>
            {data.options.map((option, i) => {
          ...
          {!isReveal && <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>{data.guessedPlayerIds.length} / {nonHostPlayers.length} answered</p>}
        </div>
      </div>
      <div style={{ background: '#111127', borderLeft: '1px solid rgba(255,255,255,0.06)', padding: '1.5rem 1rem', overflowY: 'auto' }}>
        <h3 style={{ fontSize: '0.85rem', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', marginBottom: '1rem' }}>Scoreboard</h3>
        {sorted.map((p, i) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: i === 0 ? '#8b5cf6' : '#6b7280', width: 20 }}>#{i + 1}</span>
            <div style={{ width: 24, height: 24, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor], flexShrink: 0 }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f0f0ff', flex: 1 }}>{p.name}</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#a78bfa' }}>{state.scores[p.id] ?? 0}</span>
          </div>
...
// ── Photo Finish Host View ────────────────────────────────────────────────────

function PhotoFinishHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as PhotoFinishData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (data.positions[b.id] ?? 0) - (data.positions[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.answerMs / 8_000;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        <span style={{ color: '#6b7280', fontSize: '0.875rem', fontWeight: 600 }}>Q{data.round}</span>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#14532d', color: '#22c55e' }}>{data.category}</span>
        <div style={{ flex: 1 }} />
        {!isReveal && <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor }}>{Math.ceil(data.answerMs / 1000)}s</span>}
        <span style={{ fontSize: '1.5rem' }}>🏁</span>
      </div>

      {/* Race track */}
      <div style={{ padding: '0 2rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {nonHostPlayers.map((p) => {
          const pos = data.positions[p.id] ?? 0;
          const pct = (pos / data.finishLine) * 100;
          const isWinner = data.winnerId === p.id;
          return (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: AVATAR_COLOR_HEX[p.avatarColor], width: 60, textAlign: 'right' }}>{p.name}</span>
              <div style={{ flex: 1, height: 24, background: '#1f2937', borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
                <div style={{ height: '100%', width: `${pct}%`, background: AVATAR_COLOR_HEX[p.avatarColor], borderRadius: 6, transition: 'width 0.5s ease-out', opacity: 0.7 }} />
                <div style={{ position: 'absolute', right: 4, top: '50%', transform: 'translateY(-50%)', fontSize: '0.65rem', color: '#9ca3af' }}>
                  {pos}/{data.finishLine}
                </div>
              </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        {!isReveal && <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>{data.answeredPlayerIds.length} / {nonHostPlayers.length} answered</p>}
      </div>
...
function CountdownHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as CountdownData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isCounting = data.phase === 'counting';
  const seconds = Math.max(0, data.countdownMs / 1000);
  const displayTime = seconds.toFixed(1);
  const isNearZero = seconds < 1;
  const isPastZero = data.countdownMs <= 0;
        ...
        {isCounting ? (
          <>
            <span style={{
              ...
              color: isPastZero ? '#ef4444' : isNearZero ? '#f59e0b' : '#22c55e',
              transition: 'color 0.3s',
              textShadow: isNearZero ? '0 0 40px currentColor' : 'none',
            }}>
              {isPastZero ? '0.0' : displayTime}
            </span>
            <p style={{ color: '#6b7280', fontSize: '1.2rem', fontWeight: 600 }}>
              {isPastZero ? 'Too late!' : 'Press at ZERO!'}
            </p>
            <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>
              {data.tappedPlayerIds.length} / {nonHostPlayers.length} tapped
            </p>
          </>
        ) : (
          <>
            <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#a78bfa' }}>Results</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', width: '100%', maxWidth: 500 }}>
              {[...nonHostPlayers]
                .sort((a, b) => Math.abs(data.playerTaps[a.id] ?? 99999) - Math.abs(data.playerTaps[b.id] ?? 99999))
                .map((p) => {
                  const tap = data.playerTaps[p.id];
                  const isPerfect = data.perfectPlayerIds.includes(p.id);
                  const missed = tap === undefined;
                  return (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 1rem', borderRadius: 10, background: isPerfect ? 'rgba(34,197,94,0.15)' : 'rgba(30,30,50,0.6)' }}>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor] }} />
                      <span style={{ fontSize: '0.9rem', fontWeight: 700, color: '#f0f0ff', flex: 1 }}>{p.name}</span>
                      <span style={{ fontSize: '0.85rem', fontWeight: 800, color: missed ? '#4b5563' : isPerfect ? '#22c55e' : tap! < 0 ? '#ef4444' : '#f59e0b' }}>
                        {missed ? 'MISSED' : isPerfect ? 'PERFECT!' : `${tap! > 0 ? '+' : ''}${tap}ms`}
                      </span>
                    </div>
                  );
                })}
...
// ── Vibe Check Host View ─────────────────────────────────────────────────────

function VibeCheckHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as VibeCheckData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.voteMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#1e293b', color: '#94a3b8' }}>{data.category}</span>
        <div style={{ flex: 1 }} />
        {!isReveal && <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor }}>{Math.ceil(data.voteMs / 1000)}s</span>}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.5rem, 3.5vw, 2.5rem)', fontWeight: 900, textAlign: 'center', color: '#f0f0ff', maxWidth: 800 }}>
          {data.question}
        </h1>
        {isReveal && data.isUnanimous && (
          <p style={{ fontSize: '1.2rem', fontWeight: 800, color: '#f59e0b' }}>UNANIMOUS! Everyone agreed!</p>
        )}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 700 }}>
          {data.options.map((option, i) => {
            const color = ANSWER_COLORS[i]!;
            const isMajority = isReveal && data.majorityIndex === i;
            const voteCount = isReveal ? (data.voteCounts[i] ?? 0) : 0;
            let bg = '#1a1a2e';
            let border = `2px solid ${color}44`;
            let opacity = 1;
            if (isReveal) {
              if (isMajority) { bg = `${color}33`; border = `2px solid ${color}`; }
              else { opacity = 0.5; }
            }
            return (
              <div key={i} style={{ padding: '1rem', borderRadius: 12, background: bg, border, opacity, textAlign: 'center', transition: 'all 0.3s', position: 'relative' }}>
                ...
                <p style={{ fontWeight: 700, fontSize: '1.1rem', color: '#f0f0ff', marginTop: '0.3rem' }}>{option}</p>
                {isReveal && (
                  <p style={{ fontSize: '0.8rem', fontWeight: 700, color: isMajority ? color : '#4b5563', marginTop: '0.25rem' }}>
                    {voteCount} vote{voteCount !== 1 ? 's' : ''} {isMajority ? '👑' : ''}
                  </p>
                )}
              </div>
            );
        ...
        {!isReveal && <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>{data.votedPlayerIds.length} / {nonHostPlayers.length} voted</p>}
        {isReveal && <p style={{ color: '#a78bfa', fontSize: '0.9rem', fontWeight: 600 }}>Match the majority to score!</p>}
      </div>
...
// ── Closest Guess Host View ──────────────────────────────────────────────────

function ClosestGuessHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as ClosestGuessData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.guessMs / 10_000;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        {isReveal && (
          <p style={{ fontSize: '2rem', fontWeight: 900, color: '#22c55e' }}>
            Answer: {data.correctAnswer?.toLocaleString()}{data.unit ? ` ${data.unit}` : ''}
          </p>
        )}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 700 }}>
          {data.options.map((option, i) => {
            const color = ANSWER_COLORS[i]!;
            const isClosest = isReveal && data.closestIndex === i;
            let bg = '#1a1a2e';
            let border = `2px solid ${color}44`;
            let opacity = 1;
            if (isReveal) { if (isClosest) { bg = `${color}33`; border = `2px solid ${color}`; } else { opacity = 0.4; } }
            return (
              <div key={i} style={{ padding: '1rem', borderRadius: 12, background: bg, border, opacity, textAlign: 'center', transition: 'all 0.3s' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: 800, color }}>{['A', 'B', 'C', 'D'][i]}</span>
                <p style={{ fontWeight: 900, fontSize: '1.5rem', color: '#f0f0ff', marginTop: '0.3rem' }}>
                  {option.toLocaleString()}{data.unit ? ` ${data.unit}` : ''}
                </p>
                {isReveal && isClosest && <span style={{ fontSize: '0.75rem', color: '#22c55e', fontWeight: 700 }}>CLOSEST</span>}
              </div>
            );
          })}
        </div>
...
// ── Blitz Quiz Host View ─────────────────────────────────────────────────────

function BlitzQuizHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as BlitzQuizData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.questionMs / 4_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        <span style={{ color: '#6b7280', fontSize: '0.875rem', fontWeight: 600 }}>Q{data.round}/{data.totalRounds}</span>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#7c3aed33', color: '#a78bfa' }}>BLITZ</span>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#1e293b', color: '#94a3b8' }}>{data.category}</span>
        <div style={{ flex: 1 }} />
        {!isReveal && <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor }}>{Math.ceil(data.questionMs / 1000)}s</span>}
        <span style={{ fontSize: '1.5rem' }}>⚡</span>
      </div>
...
// ── Synonym Sprint Host View ─────────────────────────────────────────────────

function SynonymSprintHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SynonymSprintData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '1rem' }}>Which word means the same as...</p>
        <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: 900, color: '#a78bfa', textAlign: 'center' }}>
          {data.word}
        </h1>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 700 }}>
          {data.options.map((option, i) => {
...
// ── Danger Zone Host View ────────────────────────────────────────────────────

function DangerZoneHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as DangerZoneData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const isReveal = data.phase === 'reveal';
  const timerFraction = data.chooseMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: isReveal && data.isDanger ? '#1a0000' : '#0a0a16', color: '#fff', transition: 'background 0.5s' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        <span style={{ color: '#6b7280', fontSize: '0.875rem', fontWeight: 600 }}>Round {data.round}/{data.totalRounds}</span>
        <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '9999px', background: '#7f1d1d', color: '#fca5a5' }}>Danger: {data.dangerChance}%</span>
        <div style={{ flex: 1 }} />
        {!isReveal && <span style={{ fontSize: '1.5rem', fontWeight: 900, color: timerColor }}>{Math.ceil(data.chooseMs / 1000)}s</span>}
        <span style={{ fontSize: '1.5rem' }}>🔥</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        {!isReveal ? (
          <>
            ...
            <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 900, textAlign: 'center', color: '#f0f0ff' }}>
              Bank your points or risk it all?
            </h1>
            <div style={{ display: 'flex', gap: '2rem' }}>
              <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: '#14532d44', border: '2px solid #22c55e44', textAlign: 'center' }}>
                <p style={{ fontWeight: 900, fontSize: '1.5rem', color: '#22c55e' }}>SAFE</p>
                <p style={{ color: '#6b7280', fontSize: '0.85rem' }}>+100 pts</p>
              </div>
              <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: '#7f1d1d44', border: '2px solid #ef444444', textAlign: 'center' }}>
                <p style={{ fontWeight: 900, fontSize: '1.5rem', color: '#ef4444' }}>RISK</p>
                <p style={{ color: '#6b7280', fontSize: '0.85rem' }}>+300 or -150</p>
              </div>
            </div>
            <p style={{ color: '#4b5563', fontSize: '0.85rem' }}>{data.chosenPlayerIds.length} / {nonHostPlayers.length} chose</p>
          </>
        ) : (
          <>
            <span style={{ fontSize: '5rem' }}>{data.eventEmoji}</span>
            <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 900, textAlign: 'center', color: data.isDanger ? '#ef4444' : '#22c55e' }}>
              {data.eventMessage}
            </h1>
            <div style={{ display: 'flex', gap: '2rem', marginTop: '1rem' }}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: '2rem', fontWeight: 900, color: '#22c55e' }}>{data.safeCount}</p>
                <p style={{ fontSize: '0.85rem', color: '#6b7280' }}>played safe</p>
              </div>
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: '2rem', fontWeight: 900, color: '#ef4444' }}>{data.riskCount}</p>
                <p style={{ fontSize: '0.85rem', color: '#6b7280' }}>took the risk</p>
              </div>
            </div>
            {data.isDanger && data.riskCount > 0 && (
              <p style={{ color: '#ef4444', fontSize: '1.1rem', fontWeight: 700 }}>Riskers lose 150 pts!</p>
            )}
            {!data.isDanger && data.riskCount > 0 && (
              <p style={{ color: '#22c55e', fontSize: '1.1rem', fontWeight: 700 }}>Riskers win 300 pts!</p>
            )}
          </>
        )}
      </div>
...
function SpellBeeHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SpellBeeData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  const difficultyColor = data.difficulty === 'easy' ? '#22c55e' : data.difficulty === 'medium' ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        <span style={{ color: '#6b7280', fontSize: '0.875rem', fontWeight: 600 }}>Round {data.round}/{data.totalRounds}</span>
        <div style={{ flex: 1, height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(data.round / data.totalRounds) * 100}%`, background: '#a78bfa', borderRadius: 4 }} />
        </div>
        <span style={{ fontSize: '1.5rem' }}>🐝</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <span style={{ fontSize: '0.9rem', fontWeight: 600, color: difficultyColor, textTransform: 'uppercase' }}>{data.difficulty}</span>
        <h1 style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700, color: '#e0e0ff', textAlign: 'center', maxWidth: 700 }}>
          &ldquo;{data.definition}&rdquo;
        </h1>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 600, marginTop: '1rem' }}>
          {data.options.map((opt, i) => {
            const isCorrect = isReveal && i === data.correctIndex;
            const isWrong = isReveal && i !== data.correctIndex;
            return (
              <div key={i} style={{
                padding: '1rem 1.2rem',
                borderRadius: 12,
                background: isCorrect ? 'rgba(34,197,94,0.2)' : isWrong ? 'rgba(100,100,120,0.15)' : 'rgba(30,30,50,0.6)',
                border: `2px solid ${isCorrect ? '#22c55e' : isWrong ? '#374151' : COLORS[i]}`,
                display: 'flex', alignItems: 'center', gap: '0.8rem',
                opacity: isWrong ? 0.4 : 1,
                transition: 'all 0.3s',
              }}>
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', fontWeight: 700 }}>{opt}</span>
              </div>
            );
          })}
        </div>
        ...
        {!isReveal && (
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', justifyContent: 'center' }}>
            {nonHostPlayers.map((p) => {
              const picked = data.pickedPlayerIds.includes(p.id);
              return (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.2rem 0.5rem', borderRadius: 6, background: picked ? 'rgba(34,197,94,0.15)' : 'rgba(30,30,50,0.4)' }}>
                  <div style={{ width: 12, height: 12, borderRadius: '50%', background: AVATAR_COLOR_HEX[p.avatarColor] }} />
                  <span style={{ fontSize: '0.75rem', fontWeight: 600, color: picked ? '#22c55e' : '#6b7280' }}>{picked ? '✓' : '...'}</span>
                </div>
              );
            })}
          </div>
...
// ── Snap Judge Host View ────────────────────────────────────────────────────

function SnapJudgeHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SnapJudgeData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 5_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  const isReveal = data.phase === 'reveal';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#6b7280' }}>{data.category}</span>
        <span style={{ fontSize: '1.5rem' }}>⚡</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center' }}>
          {data.question}
        </h1>

        <div style={{ display: 'flex', gap: '2rem', width: '100%', maxWidth: 700, justifyContent: 'center', alignItems: 'stretch' }}>
          {/* Option A */}
          <div style={{
            flex: 1, padding: '1.5rem', borderRadius: 16, textAlign: 'center',
            background: isReveal && data.correctAnswer === 'A' ? 'rgba(34,197,94,0.2)' : isReveal ? 'rgba(100,100,120,0.1)' : 'rgba(239,68,68,0.15)',
            border: `3px solid ${isReveal && data.correctAnswer === 'A' ? '#22c55e' : isReveal ? '#374151' : '#ef4444'}`,
            opacity: isReveal && data.correctAnswer !== 'A' ? 0.5 : 1,
            transition: 'all 0.3s',
          }}>
            <span style={{ fontSize: '1.5rem', fontWeight: 900, color: isReveal && data.correctAnswer === 'A' ? '#22c55e' : '#ef4444' }}>A</span>
            <p style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', fontWeight: 700, marginTop: '0.5rem' }}>{data.itemA}</p>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', fontSize: '1.5rem', fontWeight: 900, color: '#6b7280' }}>VS</div>

          {/* Option B */}
          <div style={{
            flex: 1, padding: '1.5rem', borderRadius: 16, textAlign: 'center',
            background: isReveal && data.correctAnswer === 'B' ? 'rgba(34,197,94,0.2)' : isReveal ? 'rgba(100,100,120,0.1)' : 'rgba(59,130,246,0.15)',
            border: `3px solid ${isReveal && data.correctAnswer === 'B' ? '#22c55e' : isReveal ? '#374151' : '#3b82f6'}`,
            opacity: isReveal && data.correctAnswer !== 'B' ? 0.5 : 1,
            transition: 'all 0.3s',
          }}>
            <span style={{ fontSize: '1.5rem', fontWeight: 900, color: isReveal && data.correctAnswer === 'B' ? '#22c55e' : '#3b82f6' }}>B</span>
            <p style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', fontWeight: 700, marginTop: '0.5rem' }}>{data.itemB}</p>
          </div>
        </div>

        {isReveal && data.funFact && (
          <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600, textAlign: 'center', maxWidth: 600 }}>
            {data.funFact}
          </p>
        )}
...
function GuessTheYearHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as GuessTheYearData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        <p style={{ fontSize: '1.1rem', color: '#a78bfa', fontWeight: 600 }}>When did it happen?</p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 600, marginTop: '0.5rem' }}>
          {data.optionLabels.map((label, i) => {
            const isCorrect = isReveal && i === data.correctIndex;
            const isWrong = isReveal && i !== data.correctIndex;
            return (
              <div key={i} style={{
                padding: '1.2rem 1.5rem', borderRadius: 12, textAlign: 'center',
                background: isCorrect ? 'rgba(34,197,94,0.2)' : isWrong ? 'rgba(100,100,120,0.15)' : 'rgba(30,30,50,0.6)',
                border: `2px solid ${isCorrect ? '#22c55e' : isWrong ? '#374151' : COLORS[i]}`,
                opacity: isWrong ? 0.4 : 1, transition: 'all 0.3s',
              }}>
                <span style={{ fontSize: '1rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <p style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700, marginTop: '0.3rem' }}>{label}</p>
              </div>
            );
          })}
        </div>
...
function AlphabetRaceHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as AlphabetRaceData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 6_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  const isReveal = data.phase === 'reveal';
  const LABELS = ['A', 'B', 'C', 'D'];
  const COLORS = ['#ef4444', '#3b82f6', '#22c55e', '#f59e0b'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <div style={{ fontSize: 'clamp(3rem, 8vw, 6rem)', fontWeight: 900, color: '#a78bfa', lineHeight: 1 }}>{data.letter}</div>
        <h1 style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700, color: '#f59e0b', textAlign: 'center' }}>
          {data.prompt}
        </h1>
...
function Top5HostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as Top5Data;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 6_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which one does NOT belong?</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 600, marginTop: '0.5rem' }}>
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                padding: '1rem 1.2rem', borderRadius: 12,
                background: isCorrect ? 'rgba(239,68,68,0.2)' : isWrong ? 'rgba(34,197,94,0.1)' : 'rgba(30,30,50,0.6)',
                border: `2px solid ${isCorrect ? '#ef4444' : isWrong ? '#22c55e' : COLORS[i]}`,
                display: 'flex', alignItems: 'center', gap: '0.8rem',
                transition: 'all 0.3s',
              }}>
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#ef4444' : isWrong ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(1rem, 2.5vw, 1.3rem)', fontWeight: 700 }}>{opt}</span>
                {isReveal && isCorrect && <span style={{ marginLeft: 'auto', fontSize: '1.2rem' }}>❌</span>}
                {isReveal && isWrong && <span style={{ marginLeft: 'auto', fontSize: '1.2rem' }}>✅</span>}
              </div>
            );
          })}
        </div>
...
function SpotTheFakeHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SpotTheFakeData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <h1 style={{ fontSize: 'clamp(1.3rem, 3vw, 2rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center' }}>
          Which &quot;fact&quot; is FAKE?
        </h1>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', width: '100%', maxWidth: 700 }}>
          {data.facts.map((fact, i) => {
            const isFake = isReveal && i === data.correctIndex;
            const isTrue = isReveal && i !== data.correctIndex;
            return (
              <div key={i} style={{
                padding: '0.9rem 1.2rem', borderRadius: 12,
                background: isFake ? 'rgba(239,68,68,0.2)' : isTrue ? 'rgba(34,197,94,0.1)' : 'rgba(30,30,50,0.6)',
                border: `2px solid ${isFake ? '#ef4444' : isTrue ? '#22c55e' : COLORS[i]}`,
                display: 'flex', alignItems: 'center', gap: '0.8rem',
                transition: 'all 0.3s',
              }}>
                <span style={{ fontSize: '1.1rem', fontWeight: 900, color: isFake ? '#ef4444' : isTrue ? '#22c55e' : COLORS[i], flexShrink: 0 }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(0.85rem, 2vw, 1.1rem)', fontWeight: 600 }}>{fact}</span>
                {isReveal && isFake && <span style={{ marginLeft: 'auto', fontSize: '1.1rem', flexShrink: 0 }}>🚫 FAKE</span>}
                {isReveal && isTrue && <span style={{ marginLeft: 'auto', fontSize: '1.1rem', flexShrink: 0 }}>✅</span>}
              </div>
            );
          })}
        </div>
        ...
        {isReveal && data.explanation && (
          <p style={{ fontSize: '0.95rem', color: '#a78bfa', fontWeight: 600, textAlign: 'center', maxWidth: 600, marginTop: '0.5rem' }}>
            {data.explanation}
          </p>
        )}
...
function ThisOrThatHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as ThisOrThatData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 5_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  const isReveal = data.phase === 'reveal';
  const totalVotes = (data.countA ?? 0) + (data.countB ?? 0);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem', padding: '0 2rem' }}>
        <div style={{ display: 'flex', gap: '2rem', width: '100%', maxWidth: 700, justifyContent: 'center', alignItems: 'stretch' }}>
          <div style={{
            flex: 1, padding: '2rem 1.5rem', borderRadius: 16, textAlign: 'center',
            background: isReveal && data.majority === 'A' ? 'rgba(34,197,94,0.2)' : isReveal && data.majority === 'B' ? 'rgba(100,100,120,0.1)' : 'rgba(239,68,68,0.15)',
            border: `3px solid ${isReveal && data.majority === 'A' ? '#22c55e' : isReveal ? '#374151' : '#ef4444'}`,
            transition: 'all 0.3s',
          }}>
            <span style={{ fontSize: '1.5rem', fontWeight: 900, color: isReveal && data.majority === 'A' ? '#22c55e' : '#ef4444' }}>A</span>
            <p style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700, marginTop: '0.5rem' }}>{data.optionA}</p>
            {isReveal && totalVotes > 0 && (
              <div style={{ marginTop: '1rem' }}>
                <div style={{ height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${((data.countA ?? 0) / totalVotes) * 100}%`, background: '#ef4444', borderRadius: 4, transition: 'width 0.5s' }} />
                </div>
                <span style={{ fontSize: '1rem', fontWeight: 700, color: '#ef4444' }}>{data.countA} vote{data.countA !== 1 ? 's' : ''}</span>
              </div>
            )}
          </div>
          ...
          <div style={{
            flex: 1, padding: '2rem 1.5rem', borderRadius: 16, textAlign: 'center',
            background: isReveal && data.majority === 'B' ? 'rgba(34,197,94,0.2)' : isReveal && data.majority === 'A' ? 'rgba(100,100,120,0.1)' : 'rgba(59,130,246,0.15)',
            border: `3px solid ${isReveal && data.majority === 'B' ? '#22c55e' : isReveal ? '#374151' : '#3b82f6'}`,
            transition: 'all 0.3s',
          }}>
            <span style={{ fontSize: '1.5rem', fontWeight: 900, color: isReveal && data.majority === 'B' ? '#22c55e' : '#3b82f6' }}>B</span>
            <p style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700, marginTop: '0.5rem' }}>{data.optionB}</p>
            {isReveal && totalVotes > 0 && (
              <div style={{ marginTop: '1rem' }}>
                <div style={{ height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${((data.countB ?? 0) / totalVotes) * 100}%`, background: '#3b82f6', borderRadius: 4, transition: 'width 0.5s' }} />
                </div>
                <span style={{ fontSize: '1rem', fontWeight: 700, color: '#3b82f6' }}>{data.countB} vote{data.countB !== 1 ? 's' : ''}</span>
              </div>
            )}
          </div>
        </div>

        {isReveal && data.unanimous && (
          <p style={{ fontSize: '1.2rem', color: '#f59e0b', fontWeight: 800 }}>UNANIMOUS! +100 bonus!</p>
        )}
...
function NumberCrunchHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as NumberCrunchData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  const LABELS = ['A', 'B', 'C', 'D'];
  const COLORS = ['#ef4444', '#3b82f6', '#22c55e', '#f59e0b'];
  const diffColor = data.difficulty === 'easy' ? '#22c55e' : data.difficulty === 'medium' ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.8rem', fontWeight: 600, color: diffColor, textTransform: 'uppercase' }}>{data.difficulty}</span>
        <span style={{ fontSize: '1.5rem' }}>🔢</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which expression equals...</p>
        <div style={{ fontSize: 'clamp(3rem, 8vw, 5rem)', fontWeight: 900, color: '#f59e0b', lineHeight: 1 }}>{data.target}</div>
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <p style={{ fontSize: 'clamp(1.2rem, 3vw, 1.6rem)', fontWeight: 700, marginTop: '0.3rem', fontFamily: 'monospace' }}>{opt}</p>
              </div>
            );
          })}
        </div>
...
function FinishTheLyricHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as FinishTheLyricData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <h1 style={{ fontSize: 'clamp(1.3rem, 3.5vw, 2.2rem)', fontWeight: 700, color: '#e0e0ff', textAlign: 'center', maxWidth: 700, fontStyle: 'italic' }}>
          &ldquo;{data.lyric}&rdquo;
        </h1>

        {isReveal && data.song && (
          <p style={{ fontSize: '0.9rem', color: '#a78bfa', fontWeight: 600 }}>
...
function CapitalQuizHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as CapitalQuizData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#6b7280' }}>{data.continent}</span>
        <span style={{ fontSize: '1.5rem' }}>🌍</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What is the capital of...</p>
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 3.5rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center' }}>
          {data.country}
        </h1>
...
function EmojiMathHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as EmojiMathData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 12_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  const isReveal = data.phase === 'reveal';
  const LABELS = ['A', 'B', 'C', 'D'];
  const COLORS = ['#ef4444', '#3b82f6', '#22c55e', '#f59e0b'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: DIFF_COLORS[data.difficulty] ?? '#6b7280', textTransform: 'uppercase' }}>{data.difficulty}</span>
        <span style={{ fontSize: '1.5rem' }}>🧮</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.2rem', padding: '0 2rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
          {data.clues.map((clue, i) => (
            <p key={i} style={{ fontSize: 'clamp(1.2rem, 3.5vw, 2rem)', fontWeight: 800, color: '#f0f0ff', margin: 0 }}>{clue}</p>
          ))}
        </div>

        <h1 style={{ fontSize: 'clamp(1.5rem, 5vw, 3rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center', margin: '0.5rem 0' }}>
          {data.question}
        </h1>
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700 }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function BeforeOrAfterHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as BeforeOrAfterData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  const isReveal = data.phase === 'reveal';
  const votesA = isReveal ? Object.values(data.playerPicks).filter((v) => v === 'A').length : 0;
  const votesB = isReveal ? Object.values(data.playerPicks).filter((v) => v === 'B').length : 0;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#a78bfa' }}>{data.category}</span>
        <span style={{ fontSize: '1.5rem' }}>⏳</span>
      </div>
      ...
      <p style={{ textAlign: 'center', fontSize: '1rem', color: '#a78bfa', fontWeight: 700, margin: '1rem 0 0' }}>Which happened first?</p>

      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <div style={{
          flex: 1, maxWidth: 320, padding: '2rem 1.5rem', borderRadius: 16, textAlign: 'center',
          background: isReveal && data.correctAnswer === 'A' ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.08)',
          border: `2px solid ${isReveal && data.correctAnswer === 'A' ? '#22c55e' : '#ef4444'}`,
        }}>
          <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#ef4444' }}>A</span>
          <p style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', fontWeight: 700, margin: '0.8rem 0 0' }}>{data.eventA}</p>
          {isReveal && data.yearA !== null && (
            <p style={{ fontSize: '1.8rem', fontWeight: 900, color: data.correctAnswer === 'A' ? '#22c55e' : '#ef4444', margin: '0.5rem 0 0' }}>{data.yearA < 0 ? `${Math.abs(data.yearA)} BC` : data.yearA}</p>
          )}
          {isReveal && totalVotes > 0 && (
            <div style={{ marginTop: '0.8rem' }}>
              <div style={{ height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(votesA / totalVotes) * 100}%`, background: '#ef4444', borderRadius: 4 }} />
              </div>
              <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>{votesA} vote{votesA !== 1 ? 's' : ''}</span>
            </div>
          )}
        </div>

        <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#6b7280' }}>VS</span>

        <div style={{
          flex: 1, maxWidth: 320, padding: '2rem 1.5rem', borderRadius: 16, textAlign: 'center',
          background: isReveal && data.correctAnswer === 'B' ? 'rgba(34,197,94,0.15)' : 'rgba(59,130,246,0.08)',
          border: `2px solid ${isReveal && data.correctAnswer === 'B' ? '#22c55e' : '#3b82f6'}`,
        }}>
          <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#3b82f6' }}>B</span>
          <p style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', fontWeight: 700, margin: '0.8rem 0 0' }}>{data.eventB}</p>
          {isReveal && data.yearB !== null && (
            <p style={{ fontSize: '1.8rem', fontWeight: 900, color: data.correctAnswer === 'B' ? '#22c55e' : '#ef4444', margin: '0.5rem 0 0' }}>{data.yearB < 0 ? `${Math.abs(data.yearB)} BC` : data.yearB}</p>
          )}
          {isReveal && totalVotes > 0 && (
            <div style={{ marginTop: '0.8rem' }}>
              <div style={{ height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(votesB / totalVotes) * 100}%`, background: '#3b82f6', borderRadius: 4 }} />
              </div>
              <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>{votesB} vote{votesB !== 1 ? 's' : ''}</span>
            </div>
          )}
        </div>
      </div>
      ...
      {!isReveal && (
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', justifyContent: 'center', padding: '0 2rem 1rem' }}>
          {nonHostPlayers.map((p) => {
            const picked = data.pickedPlayerIds.includes(p.id);
            return (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.2rem 0.5rem', borderRadius: 6, background: picked ? 'rgba(34,197,94,0.15)' : 'rgba(30,30,50,0.4)' }}>
...
function RhymeTimeHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as RhymeTimeData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What rhymes with...</p>
        <h1 style={{ fontSize: 'clamp(2.5rem, 8vw, 5rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center' }}>
          {data.word}
        </h1>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 500, marginTop: '0.5rem' }}>
          {data.options.map((opt, i) => {
            const isCorrect = isReveal && i === data.correctIndex;
            const isWrong = isReveal && i !== data.correctIndex;
            return (
...
function MissingLetterHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as MissingLetterData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#a78bfa' }}>{data.category}</span>
        <span style={{ fontSize: '1.5rem' }}>🔤</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Fill in the missing letter</p>
        <h1 style={{ fontSize: 'clamp(2rem, 7vw, 4.5rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', letterSpacing: '0.15em' }}>
          {isReveal ? data.fullWord : data.display}
        </h1>
        {isReveal && (
          <p style={{ fontSize: '1.2rem', fontWeight: 700, color: '#22c55e', margin: 0 }}>
            {data.fullWord}
          </p>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 400 }}>
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                padding: '1.2rem', borderRadius: 12, textAlign: 'center',
                background: isCorrect ? 'rgba(34,197,94,0.2)' : isWrong ? 'rgba(100,100,120,0.15)' : 'rgba(30,30,50,0.6)',
                border: `2px solid ${isCorrect ? '#22c55e' : isWrong ? '#374151' : COLORS[i]}`,
                opacity: isWrong ? 0.4 : 1, transition: 'all 0.3s',
              }}>
                <span style={{ fontSize: '0.8rem', fontWeight: 800, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 900, marginLeft: '0.5rem' }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function SoundAlikeHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SoundAlikeData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which spelling means...</p>
        <h1 style={{ fontSize: 'clamp(1.8rem, 5vw, 3rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center' }}>
          {data.clue}
        </h1>
        <p style={{ fontSize: '0.9rem', color: '#6b7280', fontStyle: 'italic' }}>{data.hint}</p>
...
function CategoryBlitzHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as CategoryBlitzData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>{data.category}</span>
          <span style={{ fontSize: '0.9rem', color: '#6b7280' }}>starting with</span>
          <span style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 900, color: '#f59e0b' }}>{data.letter}</span>
        </div>
...
function AcronymDecoderHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as AcronymDecoderData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What does it stand for?</p>
        <h1 style={{ fontSize: 'clamp(2.5rem, 8vw, 5rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center', letterSpacing: '0.1em' }}>
          {data.acronym}
        </h1>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', width: '100%', maxWidth: 650, marginTop: '0.5rem' }}>
          {data.options.map((opt, i) => {
            const isCorrect = isReveal && i === data.correctIndex;
            const isWrong = isReveal && i !== data.correctIndex;
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i], flexShrink: 0 }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(0.75rem, 1.8vw, 1rem)', fontWeight: 600 }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function BiggerOrSmallerHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as BiggerOrSmallerData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <p style={{ textAlign: 'center', fontSize: '1rem', color: '#a78bfa', fontWeight: 700, margin: '1rem 0 0' }}>Which is bigger?</p>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <div style={{
          ...
          <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#ef4444' }}>A</span>
          <p style={{ fontSize: 'clamp(0.9rem, 2vw, 1.2rem)', fontWeight: 700, margin: '0.8rem 0 0' }}>{data.itemA}</p>
          {isReveal && data.valueA !== null && (
            <p style={{ fontSize: '1.6rem', fontWeight: 900, color: data.correctAnswer === 'A' ? '#22c55e' : '#ef4444', margin: '0.5rem 0 0' }}>
              {formatValue(data.valueA)} {data.unit}
            </p>
          )}
          {isReveal && totalVotes > 0 && (
            <div style={{ marginTop: '0.8rem' }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#3b82f6' }}>B</span>
          <p style={{ fontSize: 'clamp(0.9rem, 2vw, 1.2rem)', fontWeight: 700, margin: '0.8rem 0 0' }}>{data.itemB}</p>
          {isReveal && data.valueB !== null && (
            <p style={{ fontSize: '1.6rem', fontWeight: 900, color: data.correctAnswer === 'B' ? '#22c55e' : '#ef4444', margin: '0.5rem 0 0' }}>
              {formatValue(data.valueB)} {data.unit}
            </p>
          )}
          {isReveal && totalVotes > 0 && (
            <div style={{ marginTop: '0.8rem' }}>
...
function FamousQuotesHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as FamousQuotesData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Who said it?</p>
        <blockquote style={{ fontSize: 'clamp(1rem, 2.5vw, 1.5rem)', fontWeight: 700, color: '#f0f0ff', textAlign: 'center', fontStyle: 'italic', maxWidth: 650, lineHeight: 1.5, margin: 0 }}>
          {data.quote}
        </blockquote>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.8rem', width: '100%', maxWidth: 600, marginTop: '0.5rem' }}>
          {data.options.map((opt, i) => {
            const isCorrect = isReveal && i === data.correctIndex;
            const isWrong = isReveal && i !== data.correctIndex;
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(0.8rem, 2vw, 1.1rem)', fontWeight: 600 }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function AnimalOrNotHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as AnimalOrNotData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 6_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <p style={{ textAlign: 'center', fontSize: '1rem', color: '#a78bfa', fontWeight: 700, margin: '1rem 0 0' }}>Real animal or made up?</p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        ...
        {isReveal && (
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: '1.8rem', fontWeight: 900, color: data.isReal ? '#22c55e' : '#ef4444' }}>
              {data.isReal ? '✅ REAL!' : '❌ FAKE!'}
            </p>
            {data.fact && <p style={{ fontSize: '1rem', color: '#6b7280', fontStyle: 'italic', maxWidth: 500 }}>{data.fact}</p>}
          </div>
        )}

        {!isReveal && (
          <div style={{ display: 'flex', gap: '2rem' }}>
            <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: 'rgba(34,197,94,0.08)', border: '2px solid #22c55e', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#22c55e' }}>A</span>
              <p style={{ fontSize: '1.2rem', fontWeight: 700, margin: '0.3rem 0 0' }}>Real</p>
            </div>
            <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: 'rgba(239,68,68,0.08)', border: '2px solid #ef4444', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#ef4444' }}>B</span>
              <p style={{ fontSize: '1.2rem', fontWeight: 700, margin: '0.3rem 0 0' }}>Fake</p>
            </div>
          </div>
        )}

        {isReveal && totalVotes > 0 && (
          <div style={{ display: 'flex', gap: '2rem', width: '100%', maxWidth: 400 }}>
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(votesA / totalVotes) * 100}%`, background: '#22c55e', borderRadius: 4 }} />
              </div>
              <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>Real: {votesA}</span>
            </div>
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ height: 6, background: '#1f2937', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${(votesB / totalVotes) * 100}%`, background: '#ef4444', borderRadius: 4 }} />
              </div>
              <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>Fake: {votesB}</span>
            </div>
          </div>
        )}
...
function WordAssociationHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as WordAssociationData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What goes with...</p>
        <h1 style={{ fontSize: 'clamp(2.5rem, 8vw, 5rem)', fontWeight: 900, color: '#f59e0b', textAlign: 'center' }}>
          {data.word}
        </h1>
...
function DecadeDetectorHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as DecadeDetectorData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which decade?</p>
        <h1 style={{ fontSize: 'clamp(1.3rem, 3.5vw, 2.2rem)', fontWeight: 800, color: '#f0f0ff', textAlign: 'center', maxWidth: 600 }}>
          {data.event}
        </h1>

        {isReveal && data.year !== null && (
          <p style={{ fontSize: '2rem', fontWeight: 900, color: '#f59e0b', margin: 0 }}>
            {data.year}
          </p>
        )}
...
function EmojiTranslateHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as EmojiTranslateData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Guess the movie!</p>
        <h1 style={{ fontSize: 'clamp(3rem, 8vw, 5rem)', fontWeight: 800, color: '#f0f0ff', textAlign: 'center' }}>
          {data.emojis}
        </h1>
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(0.85rem, 2vw, 1.2rem)', fontWeight: 700 }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function SpeedMathHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SpeedMathData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 6_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Solve it!</p>
        <h1 style={{ fontSize: 'clamp(2.5rem, 7vw, 4.5rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', fontFamily: 'monospace' }}>
          {data.question} = ?
        </h1>
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', fontWeight: 700, fontFamily: 'monospace' }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function FlagFrenzyHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as FlagFrenzyData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.5rem 2rem', flexShrink: 0 }}>
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 600, color: '#a78bfa' }}>{data.continent}</span>
        <span style={{ fontSize: '1.5rem' }}>🏳️</span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which country?</p>
        <h1 style={{ fontSize: 'clamp(5rem, 12vw, 8rem)', textAlign: 'center' }}>
          {data.flag}
        </h1>
...
function OddWordOutHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as OddWordOutData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which one doesn&apos;t belong?</p>

        {isReveal && data.connection && (
          <p style={{ fontSize: '1.1rem', fontWeight: 700, color: '#f59e0b', margin: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 500, marginTop: '0.5rem' }}>
          {data.words.map((word, i) => {
            ...
            return (
              <div key={i} style={{
                padding: '1.2rem 1.4rem', borderRadius: 12,
                background: isCorrect ? 'rgba(239,68,68,0.2)' : isWrong ? 'rgba(34,197,94,0.1)' : 'rgba(30,30,50,0.6)',
                border: `2px solid ${isCorrect ? '#ef4444' : isWrong ? '#22c55e40' : COLORS[i]}`,
                display: 'flex', alignItems: 'center', gap: '0.8rem',
                transition: 'all 0.3s',
              }}>
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#ef4444' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', fontWeight: 700 }}>{word}</span>
                {isCorrect && <span style={{ marginLeft: 'auto', fontSize: '1rem' }}>❌</span>}
                {isWrong && isReveal && <span style={{ marginLeft: 'auto', fontSize: '1rem' }}>✓</span>}
              </div>
            );
          })}
        </div>
...
function TongueTwisterHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as TongueTwisterData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Complete the tongue twister!</p>
        <h1 style={{ fontSize: 'clamp(1.2rem, 3vw, 2rem)', fontWeight: 800, color: '#f0f0ff', textAlign: 'center', maxWidth: 650, fontStyle: 'italic' }}>
          &ldquo;{isReveal && data.full ? data.full : data.blanked}&rdquo;
        </h1>
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(0.9rem, 2.5vw, 1.3rem)', fontWeight: 700 }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function ColorSpyHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as ColorSpyData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 5_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What color is the TEXT?</p>
        <h1 style={{ fontSize: 'clamp(4rem, 10vw, 7rem)', fontWeight: 900, textAlign: 'center', color: data.wordHex, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          {data.word}
        </h1>
...
function ReverseSpellHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as ReverseSpellData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What word is this backwards?</p>
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', fontFamily: 'monospace', letterSpacing: '0.15em' }}>
          {data.reversed}
        </h1>
...
function WhoSaidItHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as WhoSaidItData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 12_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Who said it?</p>
        <h1 style={{ fontSize: 'clamp(1.1rem, 3vw, 1.8rem)', fontWeight: 700, color: '#f0f0ff', textAlign: 'center', maxWidth: 650, fontStyle: 'italic', lineHeight: 1.5 }}>
          ...
          {data.options.map((opt, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '1.2rem', fontWeight: 900, color: isCorrect ? '#22c55e' : COLORS[i] }}>{LABELS[i]}</span>
                <span style={{ fontSize: 'clamp(0.75rem, 1.8vw, 1.1rem)', fontWeight: 700 }}>{opt}</span>
              </div>
            );
          })}
        </div>
...
function SoundItOutHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SoundItOutData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What word is this?</p>
        <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 3rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', fontFamily: 'monospace', letterSpacing: '0.05em' }}>
          {data.phonetic}
        </h1>
...
function FirstAndLastHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as FirstAndLastData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What&apos;s the word?</p>
        <h1 style={{ fontSize: 'clamp(2.5rem, 7vw, 5rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', fontFamily: 'monospace', letterSpacing: '0.2em' }}>
          {data.hint}
        </h1>
...
function MythOrFactHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as MythOrFactData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  const isReveal = data.phase === 'reveal';
  const totalVotes = data.mythCount + data.factCount;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <h1 style={{ fontSize: 'clamp(1.2rem, 3vw, 2rem)', fontWeight: 800, color: '#f0f0ff', textAlign: 'center', maxWidth: 650, lineHeight: 1.5 }}>
          {data.statement}
        </h1>

        {isReveal && (
          <>
            <p style={{ fontSize: '2rem', fontWeight: 900, color: data.isFact ? '#22c55e' : '#ef4444', margin: 0 }}>
              {data.isFact ? '✅ FACT!' : '❌ MYTH!'}
            </p>
            {data.explanation && (
              <p style={{ fontSize: '0.95rem', color: '#a78bfa', textAlign: 'center', maxWidth: 550, lineHeight: 1.5, margin: 0 }}>
                {data.explanation}
              </p>
            )}
          </>
        ...
        {!isReveal && (
          <div style={{ display: 'flex', gap: '2rem', marginTop: '1rem' }}>
            <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: 'rgba(239,68,68,0.15)', border: '2px solid #ef4444', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#ef4444' }}>MYTH</span>
            </div>
            <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: 'rgba(34,197,94,0.15)', border: '2px solid #22c55e', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#22c55e' }}>FACT</span>
            </div>
          </div>
        )}

        {isReveal && totalVotes > 0 && (
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', width: '100%', maxWidth: 400 }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#ef4444', minWidth: 40 }}>Myth {data.mythCount}</span>
            <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden', display: 'flex' }}>
              <div style={{ width: `${(data.mythCount / totalVotes) * 100}%`, background: '#ef4444', borderRadius: 4, transition: 'width 0.3s' }} />
              <div style={{ width: `${(data.factCount / totalVotes) * 100}%`, background: '#22c55e', borderRadius: 4, transition: 'width 0.3s' }} />
            </div>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#22c55e', minWidth: 40 }}>Fact {data.factCount}</span>
          </div>
        )}
...
function LetterCountHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as LetterCountData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 6_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>How many letters?</p>
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', letterSpacing: '0.15em' }}>
          {data.word}
        </h1>

        {isReveal && data.letterCount !== null && (
          <p style={{ fontSize: '1.5rem', fontWeight: 900, color: '#f59e0b', margin: 0 }}>
            {data.letterCount} letters
          </p>
        )}

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', width: '100%', maxWidth: 400, marginTop: '0.5rem' }}>
          {data.options.map((opt, i) => {
            const isCorrect = isReveal && i === data.correctIndex;
            const isWrong = isReveal && i !== data.correctIndex;
            return (
...
function EmojiChainHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as EmojiChainData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 10_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>What word links these emojis?</p>
        <h1 style={{ fontSize: 'clamp(3rem, 8vw, 5rem)', textAlign: 'center' }}>
          {data.emojis}
        </h1>
...
function CapitalConfusionHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as CapitalConfusionData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Is this the capital?</p>
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center' }}>
          {data.city}
        </h1>
        <p style={{ fontSize: '1.2rem', color: '#6b7280', fontWeight: 600, margin: 0 }}>
          of {data.country}
        </p>

        {isReveal && (
          <>
            <p style={{ fontSize: '2rem', fontWeight: 900, color: data.isCapital ? '#22c55e' : '#ef4444', margin: 0 }}>
              {data.isCapital ? '✅ YES!' : '❌ NO!'}
            </p>
            {!data.isCapital && data.realCapital && (
              <p style={{ fontSize: '1.2rem', fontWeight: 700, color: '#f59e0b', margin: 0 }}>
                The capital is {data.realCapital}
              </p>
            )}
            {data.funFact && (
              <p style={{ fontSize: '0.9rem', color: '#a78bfa', textAlign: 'center', maxWidth: 500, lineHeight: 1.5, margin: 0 }}>
                {data.funFact}
              </p>
            )}
          </>
        ...
        {!isReveal && (
          <div style={{ display: 'flex', gap: '2rem', marginTop: '1rem' }}>
            <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: 'rgba(34,197,94,0.15)', border: '2px solid #22c55e', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#22c55e' }}>YES</span>
            </div>
            <div style={{ padding: '1.5rem 3rem', borderRadius: 16, background: 'rgba(239,68,68,0.15)', border: '2px solid #ef4444', textAlign: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 900, color: '#ef4444' }}>NO</span>
            </div>
          </div>
        )}
        ...
        {isReveal && totalVotes > 0 && (
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', width: '100%', maxWidth: 400 }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#22c55e', minWidth: 40 }}>Yes {data.yesCount}</span>
            <div style={{ flex: 1, height: 8, background: '#1f2937', borderRadius: 4, overflow: 'hidden', display: 'flex' }}>
              <div style={{ width: `${(data.yesCount / totalVotes) * 100}%`, background: '#22c55e', borderRadius: 4, transition: 'width 0.3s' }} />
              <div style={{ width: `${(data.noCount / totalVotes) * 100}%`, background: '#ef4444', borderRadius: 4, transition: 'width 0.3s' }} />
            </div>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#ef4444', minWidth: 40 }}>No {data.noCount}</span>
          </div>
        )}
...
function SyllableCountHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as SyllableCountData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 7_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>How many syllables?</p>
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 900, color: '#f0f0ff', textAlign: 'center', letterSpacing: '0.1em' }}>
          {data.word}
        </h1>

        {isReveal && data.breakdown && (
          <p style={{ fontSize: '1.3rem', fontWeight: 700, color: '#f59e0b', margin: 0, fontFamily: 'monospace' }}>
            {data.breakdown}
          </p>
        )}
...
function OddEmojiOutHostView({ state, players }: { state: GameState; players: Player[] }) {
  const data = state.data as OddEmojiOutData;
  const nonHostPlayers = players.filter((p) => !p.isHost);
  const sorted = [...nonHostPlayers].sort((a, b) => (state.scores[b.id] ?? 0) - (state.scores[a.id] ?? 0));
  const timerFraction = data.pickMs / 8_000;
  const timerColor = timerFraction > 0.5 ? '#22c55e' : timerFraction > 0.25 ? '#f59e0b' : '#ef4444';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a16', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', padding: '0 2rem' }}>
        <p style={{ fontSize: '1rem', color: '#a78bfa', fontWeight: 600 }}>Which emoji doesn&apos;t belong?</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', width: '100%', maxWidth: 400 }}>
          {data.emojis.map((emoji, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '0.9rem', fontWeight: 900, color: isCorrect ? '#ef4444' : COLORS[i] }}>{LABELS[i]}</span>
                {isCorrect && <span style={{ fontSize: '0.8rem' }}>❌ Odd one!</span>}
              </div>
            );
          })}
        </div>
